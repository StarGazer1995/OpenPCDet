...
class VoxelSetAbstractionMRG(nn.Module):
    def __init__(self, model_cfg, voxel_size, point_cloud_range, num_bev_features=None,
        ...
        for src_name in self.model_cfg.FEATURES_SOURCE:
            ...
            cur_layer = pointnet2_stack_modules.StackSAModuleMRG(
        ...
        if 'raw_points' in self.model_cfg.FEATURES_SOURCE:
            ...
            self.SA_rawpoints = pointnet2_stack_modules.StackSAModuleMRG(
    ...
    def get_sampled_points(self, batch_dict):  #TODO: Why calculate all keypoints?
        '''

        :param batch_dict:
        :return:
        :Algorithm:
                use FPS algorithm to generate the sampled keypoint
        '''
    ...
    def forward(self, batch_dict):
        ...
        for i in range(len(keys)):
            if key[i] == 'bev' and 'bev' in self.model_cfg.FEATURES_SOURCE:
                keypoints = multi_scale_3d_features[key[i]]
                point_bev_features = self.interpolate_from_bev_features(
                    keypoints, batch_dict['spatial_features'], batch_dict['batch_size'],
                    bev_stride=batch_dict['spatial_features_stride']
                )
                point_features_list.append(point_bev_features)
            elif key[i] == 'raw_point':  #WARN: There may have an error.
                if 'raw_point' in self.model_cfg.FEATURES_SOURCE:
                    raw_points = multi_scale_3d_features[key[i]]
                    xyz = raw_points[:,1:4]
                    xyz_batch_cnt = xyz.new_zeros(batch_size).int()

                    cur_coords = multi_scale_3d_features['x_conv1'].indices
                    ...
                    pooled_point,pooled_features = self.SA_rawpoints(
                        xyz=xyz.contiguous(),
                        xyz_batch_cnt=xyz_batch_cnt,
                        new_xyz=new_xyz,
                        new_xyz_batch_cnt=new_xyz_batch_cnt
                    )
                    point_features_list.append(pooled_features.view(batch_size, num_keypoint, -1))
                else:
                    raise NotImplementedError
            else:
