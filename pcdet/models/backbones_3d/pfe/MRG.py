    ...
    def forward(self, batch_dict):
        ...
        for slow in range(len(self.model_cfg.FEATURES_SOURCE)-1):
            ...
            if self.model_cfg.FEATURES_SOURCE[slow] == 'raw_points':
                batch_size = batch_dict['batch_size']
                raw_point = batch_dict['points']
                xyz = raw_point[:, 1:4]
                xyz_batch_cnt = xyz.new_zeros(batch_size).int()

                for bs_idx in range(batch_size):
                    xyz_batch_cnt[bs_idx] = (raw_point[:, 0] == bs_idx).sum()
                point_features = raw_point[:,4:].contiguous() if len(raw_point>4) else None
                new_xyz = batch_dict['multi_scale_3d_features']['x_conv1']
                new_xyz_batch_cnt = new_xyz.new_zeros(batch_size).int().fill_(batch_size)
