...
class StackSAModuleMRG(nn.Module):
    def __init__(self,*, radii: List[float], nsamples: List[int], mlps: List[List[int]],
                 use_xyz: bool = True, pool_method='max_pool'):
        '''
        Args:
            ...
            pool_method:
        Algorithm:
            According to the PointNet++ document, the features in the Level $L_i$ is a concatenation of two vectors.
            One of the vectors is obtained in the lower level while the another one is the higher level.
        '''
        super(StackSAModuleMRG, self).__init__()
        assert len(radii) == len(nsamples) == len(mlps) #Check if the data is correct.
