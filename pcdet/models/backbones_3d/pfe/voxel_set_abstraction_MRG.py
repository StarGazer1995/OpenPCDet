...
from voxel_set_abstraction import bilinear_interpolate_torch as bilinear

def bilinear_interpolate_torch(im,x,y):
    return bilinear(im,x,y)
    ...
    def __init__(self, model_cfg, voxel_size, point_cloud_range, num_bev_features=None,
        ...
        for src_name in self.model_cfg.FEATURES_SOURCE:
            if src_name in ['bev', 'raw_points']:
                continue                                                            ## skip if the src is bev or rawpoint
            self.downsample_times_map[src_name] = SA_cfg[src_name].DOWNSAMPLE_FACTOR
            mlps = SA_cfg[src_name].MLPS                                            ## TODO: Why every level has MLP? It is the ketpoint weight module?
            for k in range(len(mlps)):
                mlps[k] = [mlps[k][0]] + mlps[k]
            '''
            It seems that the MRG module has be written by ourselves. Holy shit!
            '''
